...
```{r}
population <- resp_body_json(response, simplifyVector = T)
```
...
population |>
  ...
  coord_flip() +
  facet_wrap(~ year) +
  labs(x = "State", y = "Population size",
       title = "State populations, 2020 and 2021")
```
...
regions <- regions_dat |> 
  ...
  rename(state_name = states, region_name = region_name) |>
  mutate(
    state_name = str_trim(state_name),
    state_name = str_replace_all(state_name, "c\\(|\\)|\"", ""),
...
We see exactly 1,000 rows because the CDC API sets a default limit of 1,000 records per request. To obtain the complete dataset, we must explicitly set a higher limit by adding the parameter $limit=10000000000 in the query.
...
cases |>
  ...
  ggplot(aes(x = date, y = cases_per100k, color = state)) +
  geom_line() +
  facet_wrap(~ region_name) +
  labs(
    title = "Number of cases per 100,000 (2020–2021)",
    x = "Date",
    y = "Cases per 100,000"
  ) 
```
